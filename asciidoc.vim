...
for s:type in g:asciidoc_fenced_languages
...
if get(g:, 'asciidoc_syntax_conceal', 0)
    ...
    syn region asciidocLink start="\%(link\|xref\|mailto\):[^:].*\ze\[" end="\[.\{-}\]" oneline keepend skipwhite contained
    syn region asciidocLink start="\%(video\|image\)::\?.*\ze\[" end="\[.\{-}\]" oneline keepend skipwhite contained
    syn match asciidocUrl "\%(http\|ftp\|irc\)s\?://\S\+\ze\%(\[.\{-}\]\)" nextgroup=asciidocUrlDescription
